	...
	function gridClearClick() {
		if(_operateType!="grid_clear"&&(_editLayer.getLayers().length>0||_drawLayer.getLayers().length>0)){
			//其他模式，且已经有修改，点击此按钮
		...
		}else if(_operateType!="grid_clear"&&_editLayer.getLayers().length==0&&_drawLayer.getLayers().length==0){
			//其他模式，没有修改，点击此按钮，直接重置，并切换到编辑模式
			confirmRest();
			gridClear();
		}else if(_operateType="grid_clear"&&_editLayer.getLayers().length>0){
			//删除网格模式，且已经有修改，点击此按钮
			createConfirmAlert("提示", "将放弃所有未保存的信息，是否确认此操作？", function() {
				//弹框如果点是，进行重置
				confirmRest();
		...
		+"<font style='color:#00BFFF;'>"+polygonEditProperty.ACT_NAME+"</font>的图形信息将被删除，是否确认此操作？", function() {
			...
			$.ajax({
				...
				success: function(result) {
					...
					if (result.status == 0) {
						showMessageTip("活动已成功被清除,正在刷新...", true);
						refreshLayers();			//刷新图层
