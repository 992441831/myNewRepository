	...
	function gridEditClick() {
		if(_operateType!="edit_grid"&&(_editLayer.getLayers().length>0||_drawLayer.getLayers().length>0)){
			//其他模式，且已经有修改，点击此按钮
			createConfirmAlert("提示", "将放弃所有未保存的信息，是否确认此操作？", function() {
				//弹框如果点是，进行重置，并切换到编辑网格模式
				confirmRest();
				gridEdit();
			},function () {
				//弹框如果点否，返回
		...
		}else if(_operateType!="edit_grid"&&_editLayer.getLayers().length==0&&_drawLayer.getLayers().length==0){
			//其他模式，没有修改，点击此按钮，直接重置，并切换到编辑模式
			confirmRest();
			gridEdit();
		}else if(_operateType="edit_grid"&&_editLayer.getLayers().length>0){
			//编辑网格模式，且已经有修改，点击此按钮
			createConfirmAlert("提示", "将放弃所有未保存的信息，是否确认此操作？", function() {
				//弹框如果点是，进行重置
				confirmRest();
			...
			//编辑网格模式，没有修改，点击此按钮，直接重置
			confirmRest();
		}
	}
	function gridEdit(){
		_operateType = "edit_grid";//操作类型设置为网格编辑
		showMapMsg("请选中一个活动开始编辑区域", false, true, false,null);//显示网格编辑提示信息
	...
	function layerClickWhenEdit(feature) {
		if(polygonEdit==null){
			showMapMsg("通过操作控制点来编辑活动区域", false, true, true,null);//显示提示信息
		...
		$.ajax({
			...
			success: function(result) {
				...
				if(result.status == 0) {//保存成功
					showMessageTip("活动区域编辑成功，正在刷新...", true);//显示保存成功的提示信息
					refreshLayers();			//刷新图层
